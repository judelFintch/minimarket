<div class="space-y-8">
    ...
    @if ($isAdmin)
        <div class="app-card">
            <div class="app-card-header">
                <div>
                    <h3 class="app-card-title">Resume admin</h3>
                    <p class="app-card-subtitle">Les alertes essentielles seulement.</p>
                </div>
            </div>
            <div class="app-card-body grid gap-3 sm:grid-cols-2 xl:grid-cols-4 text-sm text-slate-700">
                <div class="rounded-2xl border border-slate-200/70 bg-slate-50 px-4 py-3">
                    <div class="text-xs uppercase tracking-wider text-slate-500">Ventes impayees</div>
                    <div class="mt-2 text-lg font-semibold text-slate-900">{{ $unpaidSalesCount }}</div>
                </div>
                <div class="rounded-2xl border border-slate-200/70 bg-slate-50 px-4 py-3">
                    <div class="text-xs uppercase tracking-wider text-slate-500">Ruptures</div>
                    <div class="mt-2 text-lg font-semibold text-slate-900">{{ $outOfStockCount }}</div>
                </div>
                <div class="rounded-2xl border border-slate-200/70 bg-slate-50 px-4 py-3">
                    <div class="text-xs uppercase tracking-wider text-slate-500">Comptes non verifies</div>
                    <div class="mt-2 text-lg font-semibold text-slate-900">{{ $unverifiedUsersCount }}</div>
                </div>
                <div class="rounded-2xl border border-slate-200/70 bg-slate-50 px-4 py-3">
                    <div class="text-xs uppercase tracking-wider text-slate-500">Produits sans prix</div>
                    <div class="mt-2 text-lg font-semibold text-slate-900">{{ $missingSalePriceCount }}</div>
                </div>
            </div>
        </div>
    @endif
